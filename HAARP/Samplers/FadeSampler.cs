        ...
        public override void Update(ref float sample)
        {
            switch (FadeType)
            {
                case FadeType.FadeIn:
                    ...
                    {
                        sample = Util.CosineInterpolate(0.0f, sample, synth.TimePosition / FadeLength);
                    }
                    break;
                case FadeType.FadeOut:
                    if (synth.Length - synth.TimePosition < FadeLength)
                    {
                        sample = Util.CosineInterpolate(0.0f, sample, (synth.Length - synth.TimePosition) / FadeLength);
                    }
                    break;
            }
        }
