	...
	internal class VocalSampler : Sampler
	{
		...
		public VocalSampler(Synthesizer synth, long seed) : base(synth)
		{
			...
			bands = new[]
			{
				...
				// ɛ
				//new BandPassFilter(2600, 2600, synth.SampleRate, HighResonance, HighResonance) {Volume = .075f},
				//new BandPassFilter(1900, 1900, synth.SampleRate, HighResonance, HighResonance) {Volume = .075f},
				//new BandPassFilter(610, 610, synth.SampleRate, LowResonance, LowResonance) {Volume = .075f},
		...
		{
			//outputSample += rng.NextSingle(-1, 1) * synth.Voice.FricativeForce * 0.06f;
